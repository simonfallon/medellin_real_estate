...
class ArrendamientosLasVegasScraper(BaseScraper):
    ...
    async def extract_property_details(self, page: Page, link: str, barrio_name: str) -> Property:
        ...
        try:
            ...
            await page.wait_for_selector("span.rounded-lg.bg-neutral-200", state="visible", timeout=10000)
        ...
        # Price
        try:
            price_label = page.locator("span", has_text=re.compile("Precio total:", re.IGNORECASE))
        ...
        # Description
        try:
            desc_header = page.locator("h3, h4, strong, span", has_text=re.compile("descripci", re.IGNORECASE)).first
        ...
        try:
            ...
            for img in imgs:
                ...
                # Exclusion keywords
                if any(x in src_lower for x in ["logo", "icon", "whatsapp", "facebook", "twitter", "instagram", "button", "arrow"]):
                    continue
