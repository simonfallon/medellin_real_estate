        ...
        async with async_playwright() as p:
            ...
            for link in links:
                try:
                    ...
                    prop = await scraper.extract_property_details(
                        detail_page, link, "TestSearch"
                    )
                    if prop:
                        results.append(prop)
            ...
            if len(results) == 0:
                print(
                    "Warning: No results found on search page. This might be due to market changes."
                )
            else:
                ...
                for result in results:
                    validate_property(
                        self, result, expected_source="arrendamientos_envigado"
                    )
