...
async function openMapModal(p) {
    ...
    // Fetch all locations to show context
    try {
        const res = await fetch(`${API_URL}/properties/locations`);
        if (res.ok) {
            const locations = await res.json();
            ...
            locations.forEach(loc => {
                // Skip the target property to avoid duplication (target will be red)
                const isTarget = (loc.id && loc.id === p.id) || (loc.latitude === p.latitude && loc.longitude === p.longitude && loc.source === p.source);
                if (isTarget) return;
        ...
        console.error("Error fetching context locations", e);
    }

    // Add target marker (Red)
    const popupContent = getMapPopupContent(p);
