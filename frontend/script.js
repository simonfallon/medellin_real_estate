...
import { fetchProperties, scrapeSource, fetchLocations } from '/static/modules/api.js?v=1';
import { filterProperties, sortProperties } from '/static/modules/filters.js?v=1';
import { renderProperties, setupBarrioFilter, initImageModal, updateResultsCount } from '/static/modules/dom.js?v=1';
...
async function loadProperties() {
    ...
    try {
        ...
        const filters = {
            ...
            areaMax: parseFloat(document.getElementById('filterAreaMax').value) || Infinity,
            bedsMin: parseFloat(document.getElementById('filterBedroomsMin').value) || 0,
            bedsMax: parseFloat(document.getElementById('filterBedroomsMax').value) || Infinity,
            parkingMin: parseFloat(document.getElementById('filterParkingMin').value) || 0,
        ...
        grid.innerHTML = '<div class="loading-state"><p>Error al cargar propiedades. Corrobora que el servidor backend esté corriendo.</p></div>';
    }
}
...
async function handleScrape() {
    ...
    try {
        ...
        if (data.cached) {
            ...
            showNotification(`Datos recientes cargados. Última actualización: ${time}`);
        } else {
            showNotification(`Búsqueda completada. ${data.new_properties} nuevas propiedades.`);
        }
