...
export function renderProperties(properties) {
  ...
  if (properties.length === 0) {
    grid.innerHTML =
      '<div class="loading-state"><p>No se encontraron propiedades con estos filtros.</p></div>';
    return;
  }
...
function createPropertyCard(p, index) {
  ...
  let normalizedSource =
    p.source === "arrendamientosenvigadosa"
      ? "arrendamientos_envigado"
      : p.source;
  ...
  card.addEventListener("click", (e) => {
    if (
      !e.target.closest(".card-image-wrapper") &&
