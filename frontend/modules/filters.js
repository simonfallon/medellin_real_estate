...
export function sortProperties(properties, sortBy) {
  ...
  return [...properties].sort((a, b) => {
    switch (sortBy) {
      ...
      case "barrio_asc":
        return getBarrioFromLocation(a.location).localeCompare(
          getBarrioFromLocation(b.location),
        );
      case "barrio_desc":
        return getBarrioFromLocation(b.location).localeCompare(
          getBarrioFromLocation(a.location),
        );
      case "created_desc":
      default:
  ...
  return properties.filter((p) => {
    ...
    if (selectedWebsite && selectedWebsite !== "all") {
      ...
      if (selected === "arrendamientos_envigado") {
        matchesSource =
          pSource === "arrendamientos_envigado" ||
          pSource === "arrendamientosenvigadosa";
      } else {
    ...
    if (selectedBarrios && !selectedBarrios.includes("all")) {
      ...
      matchesBarrio = selectedBarrios.some((b) =>
        pLoc.includes(b.toLowerCase()),
      );
    }
