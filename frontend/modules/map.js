...
import { getSourceName, formatPrice, getBarrioFromLocation } from '/static/utils.js?v=1';
...
export function openSingleMapModal(p, allLocations = []) {
    ...
    if (allLocations.length > 0) {
        ...
        allLocations.forEach(loc => {
            const isTarget = (loc.id && loc.id === p.id) || (loc.latitude === p.latitude && loc.longitude === p.longitude);
            if (isTarget) return;
    ...
    // Target marker
    marker = L.marker([p.latitude, p.longitude], { icon: redIcon }).addTo(map)
